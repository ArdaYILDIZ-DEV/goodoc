...
# Injected into every generated page so fenced code blocks get a copy button.
# Mirrors the markup/styling expected by retro-doc.css `.copy-btn`.
COPY_BUTTON_SCRIPT = """\
(function () {
  "use strict";
  // Copy code to clipboard, with a legacy fallback for non-secure contexts.
  ...
  function makeButton(pre) {
    ...
    btn.innerHTML =
      ...
      '<svg class="ic ic-check" viewBox="0 0 24 24" fill="none" stroke="currentColor"' +
        ' stroke-width="2.5" stroke-linecap="square" stroke-linejoin="miter" aria-hidden="true">' +
